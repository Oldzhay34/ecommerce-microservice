...
export const Button = ({ children, onClick, variant = 'primary', type = 'button' }: { children: React.ReactNode, onClick?: () => void, variant?: 'primary' | 'danger' | 'secondary', type?: 'button' | 'submit' }) => {
    ...
    const variants = {
        primary: "bg-brand text-white hover:bg-brand-hover rounded-full",
        danger: "bg-danger text-white hover:bg-danger/90 rounded-sb",
        secondary: "bg-surface-raised text-ink hover:bg-surface-hover rounded-sb"
    };
