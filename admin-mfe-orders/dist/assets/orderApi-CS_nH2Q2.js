...
const AuthTokenProvider = ({ token, children }) => {
...
const createApiClient = (tokenGetter) => {
...
const makeOrderApi = (client) => {
  return {
    getOrders: async (page, size = 10) => {
      const response = await client.get(ENDPOINTS.orders, {
        params: { page, size, sort: "createdAt,desc" }
      });
      return response.data;
    },
    updateStatus: async (id, status) => {
      const response = await client.patch(`${ENDPOINTS.orders}/${id}/status`, {
        status
      });
      return response.data;
    }
...
export { AuthTokenProvider as A, createApiClient as c, makeOrderApi as m, useAuthToken as u };
